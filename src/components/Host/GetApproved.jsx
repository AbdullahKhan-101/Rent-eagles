...
const GetApproved = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    liscense_photo: host?.liscense_photo || null,
    id_card_photo: host?.id_card_photo || null,
    insurance_card_photo: host?.insurance_card_photo || null,
    registration_card_photo: host?.registration_card_photo || null,
    is_mob_notifications: host?.is_mob_notifications || false,
    is_email_notifications: host?.is_email_notifications || false,
    is_driver_expert: String(host?.is_driver_expert) || "false",
    address: host?.address || "",
    place_id: host?.place_id || false,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isSelected) {
      ...
      try {
        // working on fixings
        const response = await API.GetApprovedDriverHost({
          ...formData,
    ...
    <div className="lg:max-w-[70%] px-4 my-12 w-full mx-auto flex flex-col justify-center items-start gap-8">
      ...
      >
        ...
        <div className="flex flex-wrap   justify-between items-center gap-6">
          ...
          <div className="flex flex-col justify-center items-start gap-5">
            <p>Upload your Insurance Card Photo</p>
            <UploadDocuments
              imagePreviews={imagePreviews}
              formData={formData}
              item={host}
      ...
      <ChangePassword from="host" isOpen={isOpen} onOpenChange={onOpenChange} />
    </div>
  );
};
