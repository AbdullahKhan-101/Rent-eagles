...
const Index = () => {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      // const response = await API.SignInHost(data);
  ...
  return (
    <div className="max-w-[80%] md:max-w-[40%] mx-auto w-full pb-10">
      ...
      <div className="flex justify-between sm:flex-row flex-col gap-1 item-center">
        <Link href="/host/sign-up" className="text-[#99000B] text-md  ">
      ...
      <ForgotPassword type={2} isModalOpen={openModal} />
      ...
      {/* <ErrorModal from="host" isOpen={isOpen} onOpenChange={onOpenChange} /> */}
      {userRole && (
        <ErrorModal
          from={userRole}
          isOpen={isOpen}
